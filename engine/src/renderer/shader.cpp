...
namespace cineris::renderer {
    ...
    Shader::Shader(const std::string& shaderFilePath) : m_Name(shaderFilePath)
    {
        std::string vertexFilePath = shaderFilePath + ".vert";
        std::string fragmentFilePath = shaderFilePath + ".frag";
    ...
    auto Shader::compile(const std::string& filepath, unsigned int type) -> unsigned int {
        ...
        if (!success) {
            ...
            throw std::runtime_error("Shader module compilation failed (at " + filepath + "): \n" + errorLog);
        }
    ...
    auto Shader::reload() -> void {
        std::string vertexFilePath = m_Name + ".vert";
        std::string fragmentFilePath = m_Name + ".frag";
